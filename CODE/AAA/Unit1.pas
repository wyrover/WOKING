...
var
  ...
  { 获取文件全路径，包含路径和文件名 }
  procedure GetFileFullPathName(var FileList: TQueue<TFileInfo>);
  begin
...
begin
  ...
  try
    ...
    begin
      { 找到第一个 USN 记录 }
      UsnRecord := PUSN(Integer(@(Buffer)) + Sizeof(Int64));
      while dwRet > 60 do
      begin
        { 获取文件名称 }
      ...
      Move(Buffer, med, Sizeof(Int64));
    end;

    { 获取文件全路径，包含路径和文件名 }
    GetFileFullPathName(Queue);
