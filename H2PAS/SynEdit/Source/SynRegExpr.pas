...
for 32 - bits Delphi only.Delphi 1 is not supported(and whos really care today ? !). {$ENDIF}
...
{$ENDIF}
      ...
      class function ParseModifiersStr(const AModifiers: RegExprString; var AModifiersInt: Integer): Boolean; // ###0.941 class function now
...
{$ENDIF}
      ...
      function Replace(AInputStr: RegExprString; const AReplaceStr: RegExprString; AUseSubstitution: Boolean{$IFDEF DefParam} = False{$ENDIF}) // ###0.946
...
{$ENDIF}
  ...
  function ReplaceRegExpr(const ARegExpr, AInputStr, AReplaceStr: RegExprString; AUseSubstitution: Boolean{$IFDEF DefParam} = False{$ENDIF}): RegExprString; // ###0.947
  // Returns AInputStr with r.e. occurencies replaced by AReplaceStr
  // If AUseSubstitution is True, then AReplaceStr will be used
  // as template for Substitution methods.
  // For example:
...
function ReplaceRegExpr(const ARegExpr, AInputStr, AReplaceStr: RegExprString; AUseSubstitution: Boolean{$IFDEF DefParam} = False{$ENDIF}): RegExprString;
begin
...
function QuoteRegExprMetaChars(const AStr: RegExprString): RegExprString;
const
  RegExprMetaSet: RegExprString = '^$.[()|?+*' + EscChar + '{' + ']}'; // - this last are additional to META.
...
begin
  ...
  try
        ...
        '(':
          begin
            ...
            begin
              ...
              if i > Len then
                Result := -1 // unbalansed '('
              else if TRegExpr.ParseModifiersStr(System.Copy(ARegExpr, i, i - i0), Modif) then
    ...
    if (ASubExprs.Count = 0) or ((Integer(ASubExprs.Objects[0]) and $FFFF) <> 1) or (((Integer(ASubExprs.Objects[0]) shr 16) and $FFFF) <> Len)
...
begin
  if (Idx >= 0) and (Idx < NSUBEXP) and Assigned(fInputString) and Assigned(startp[Idx]) and Assigned(endp[Idx])
...
{$IFDEF SynRegUniCode} // ###0.935
              if (ord((regparse + 1)^) < 256) and (ansichar((regparse + 1)^) in ['d', 'D', 's', 'S', 'w', 'W']) then
              begin
{$ELSE}
...
{$ENDIF}
    ...
    begin
      ...
      if ((fCompModifiers and MaskModX) <> 0) and                                             // check for eXtended syntax
        ((regparse^ = '#') or ({$IFDEF SynRegUniCode}StrScan(XIgnoredChars, regparse^) <> nil // ###0.947
...
begin
  ...
  begin
    ...
    case scan^ of
      ...
      BOUND:
        if (scan^ = BOUND) xor (((reginput = fInputStart) or (Pos((reginput - 1)^, fWordChars) <= 0)) and (reginput^ <> #0) and (Pos(reginput^, fWordChars) > 0) or (reginput <> fInputStart) and (Pos((reginput - 1)^, fWordChars) > 0) and ((reginput^ = #0) or (Pos(reginput^, fWordChars) <= 0))) then
          Exit;
      ...
      BOLML:
        ...
        begin
          nextch := (reginput - 1)^;
          if (nextch <> fLinePairedSeparatorTail) or ((reginput - 1) <= fInputStart) or ((reginput - 2)^ <> fLinePairedSeparatorHead) then
          begin
...
{$ENDIF}
      ...
      EOLML:
        ...
        begin
          ...
          begin
            if (nextch = fLinePairedSeparatorTail) and (reginput > fInputStart) and ((reginput - 1)^ = fLinePairedSeparatorHead) then
              Exit; // don't stop between paired separator
            if
{$IFNDEF SynRegUniCode}
              not(nextch in FLineSeparatorsSet)
...
{$ENDIF}
      ...
      ANYML:
        begin // ###0.941
          if (reginput^ = #0) or ((reginput^ = fLinePairedSeparatorHead) and ((reginput + 1)^ = fLinePairedSeparatorTail)) or {$IFNDEF SynRegUniCode} (reginput^ in FLineSeparatorsSet)
      ...
      ANYOFTINYSET:
        begin
          if (reginput^ = #0) or // !!!TinySet
            ((reginput^ <> (scan + REOpSz + RENextOffSz)^) and (reginput^ <> (scan + REOpSz + RENextOffSz + 1)^) and (reginput^ <> (scan + REOpSz + RENextOffSz + 2)^)) then
            Exit;
          Inc(reginput);
        end;
      ANYBUTTINYSET:
        begin
          if (reginput^ = #0) or // !!!TinySet
            (reginput^ = (scan + REOpSz + RENextOffSz)^) or (reginput^ = (scan + REOpSz + RENextOffSz + 1)^) or (reginput^ = (scan + REOpSz + RENextOffSz + 2)^) then
            Exit;
          Inc(reginput);
        end;
{$IFDEF UseSetOfChar} // ###0.929
      ANYOFFULLSET:
        begin
          if (reginput^ = #0) or not(reginput^ in PSetOfREChar(scan + REOpSz + RENextOffSz)^) then
            Exit;
          Inc(reginput);
        end;
{$ELSE}
...
function TRegExpr.Replace(AInputStr: RegExprString; const AReplaceStr: RegExprString; AUseSubstitution: Boolean{$IFDEF DefParam} = False{$ENDIF}): RegExprString;
var
  PrevPos: Integer;
begin
  Result  := '';
...
{$IFDEF ComplexBraces}
    ...
    begin // ###0.940
      Result := Result + Format(' -> (%d) {%d,%d}', [(s - programm - (REOpSz + RENextOffSz)) + PRENextOff(s + 2 * REBracesArgSz)^, PREBracesArg(s)^, PREBracesArg(s + REBracesArgSz)^]);
